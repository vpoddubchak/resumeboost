...
describe('BookingPageContent', () => {
  ...
  it('shows existing booking when user already has one', async () => {
    mockUseSession.mockReturnValue({
      data: { user: { userId: 1, email: 'test@test.com' } },
      status: 'authenticated',
    });
    ...
    mockFetch.mockImplementation(async (url: string) => {
      ...
      if (typeof url === 'string' && url.includes('/my-booking')) {
        return { ok: true, json: async () => ({ success: true, data: { booking: { consultationId: 99, date: '2026-03-17', time: '2026-03-17T08:00:00.000Z', endTime: '2026-03-17T08:30:00.000Z', timezone: 'UTC' } } }) };
      }
    ...
    expect(screen.getByText('Cancel & Reschedule')).toBeInTheDocument();
  });
